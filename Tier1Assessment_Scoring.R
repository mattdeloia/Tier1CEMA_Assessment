...
#%>%
  filter(Trait %in% c("DepthofThought", "Conscientiousness", "Teamwork", "Leadership", "InterpersonalUnderstanding", "Resilience")) 
...
df_scored %>% filter(Miss_P<.20, infreq <1) %>% 
  ...
  theme(legend.title = element_text(color="black", size=12), legend.position = "top", axis.text = element_text(size=10),
       ...
       # ggtitle("Boxplot: Average Scores by Personality Dimension") +
        ...
        ggsave("Personality_RawScores.png", width = 4.66, height = 5.53, units = "in" ) #save to folder
...
ICAR_metrics <- data.frame(Dimension, Score)
        ...
        ggsave("Cognitive_RawScores.png", width = 4.66, height = 5.4/2, units = "in" ) #save to folder
...
df_scored2 <- df_scored %>% ungroup() %>% filter(Miss_P <miss_limit, infreq<1) %>% 
...
df_scored2_summary %>% ungroup() %>% 
  ...
  mutate(Significant = if_else(p<.05, "p<0.05", if_else(p<0.1, "p<0.1", "no"))) %>%
  mutate(Significant = factor(Significant, levels=c("p<0.05", "p<0.1","no"))) %>% 
  ...
  labs(caption = "Hypothesis Test: p < 0.05 (green bars); p < 0.1 (blue bars); p >= 0.1 (gray)") +
  ggsave("PersonalityScores_WorkRole.png", width = 4.66, height = 5.53, units = "in") #save to folder
...
df_scored3_summary %>% left_join(df_scored3_order)  %>%
        filter(N>5) %>% filter(Test %in% c("Proficiency", "Cog_tot")) %>% 
        ...
        geom_errorbar(aes(ymin=Score-ci, ymax=Score+ci), width=.1 )+ 
        coord_flip() + xlab(" ") + ylab("proficiency & test mean scaled score ") +
        scale_color_manual(values=c("red", "skyblue", "darkgray", "darkgray")) +
  theme(legend.title = element_text(color="black", size=12), legend.position = "top", axis.text = element_text(size=12),
        axis.title = element_text(size=12),
        legend.text = element_text(size=12),
        strip.text = element_text(size=12, color="blue")) +
       ...
       # ggtitle("Cognitive Scores by Work Role") +
        ggsave("CognitveScores_WorkRole.png", width = 4.66, height = 5.4/2, units = "in" ) #save to folder
